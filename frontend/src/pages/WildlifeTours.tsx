  ...
  useEffect(() => {
    const loadTours = async () => {
      try {
        ...
        setError(t("wildlifeTours.errorMessage"));
      } finally {
        setIsLoading(false);
      }
    };
    ...
    <div className="min-h-screen bg-gradient-to-br from-gray-100 to-gray-200">
      ...
      >
        ...
        <div className="relative z-10">
          <h1 className="text-5xl md:text-6xl font-extrabold text-green-800 drop-shadow-md">
            {t("wildlifeTours.title")}
          </h1>
          <p className="mt-4 text-lg md:text-xl text-gray-700 max-w-3xl mx-auto">
            {t("wildlifeTours.subtitle")}
          </p>
        </div>
      </motion.div>
      ...
      >
        <h2 className="text-4xl font-extrabold text-green-800">
          {t("wildlifeTours.highlightsTitle")}
        </h2>
        <p className="mt-2 text-lg text-gray-700">
          {t("wildlifeTours.highlightsSubtitle")}
        </p>
        <div className="mt-8 grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-8">
          <div className="bg-white p-6 rounded-xl shadow-md hover:shadow-xl transition-all duration-300 transform hover:-translate-y-1">
            <h3 className="text-xl font-semibold text-green-800">
              {t("wildlifeTours.safariTitle")}
            </h3>
            <p className="text-gray-700 mt-2">
              {t("wildlifeTours.safariDescription")}
            </p>
          </div>
          <div className="bg-white p-6 rounded-xl shadow-md hover:shadow-xl transition-all duration-300 transform hover:-translate-y-1">
            <h3 className="text-xl font-semibold text-green-800">
              {t("wildlifeTours.birdwatchingTitle")}
            </h3>
            <p className="text-gray-700 mt-2">
              {t("wildlifeTours.birdwatchingDescription")}
            </p>
          </div>
          <div className="bg-white p-6 rounded-xl shadow-md hover:shadow-xl transition-all duration-300 transform hover:-translate-y-1">
            <h3 className="text-xl font-semibold text-green-800">
              {t("wildlifeTours.parksTitle")}
            </h3>
            <p className="text-gray-700 mt-2">
              {t("wildlifeTours.parksDescription")}
            </p>
          </div>
        </div>
      </motion.div>
      ...
      >
        ...
        {isLoading ? (
          ...
          <div className="text-center text-gray-700 text-lg font-semibold">
            {t("wildlifeTours.noToursMessage")}
          </div>
        ) : (
